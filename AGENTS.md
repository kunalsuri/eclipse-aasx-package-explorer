...
Instructions for AI coding agents (Claude Code, Codex CLI, Cursor, Copilot,
Amp, Jules, Aider, and similar) working in this repository. This file is
the canonical, tool-agnostic entry point — tool-specific files
(`CLAUDE.md`, `.github/copilot-instructions.md`, `.cursor/rules/*`) exist
alongside it and point back here rather than duplicating it. See
"Multi-tool setup" at the bottom for how those fit together.
...
(`AasxPlugin*` projects), and the app embeds a scripting engine for
automation.
...
(`src/AasxPackageExplorer.sln`, ~60 projects). Key projects, grouped by
role:

**Domain model / package format**
- `AasCore.Aas3_1` — the AAS v3.1 metamodel (generated data classes: `Aas.AssetAdministrationShell`, `Aas.IReferable`, etc.). Treat as generated/vendored; don't hand-edit.
- `AasxCsharpLibrary` — reads/writes `.aasx` package files (an OPC/ZIP-based container format) and the in-memory environment (`AdminShellPackageEnv`).
- `AasxPredefinedConcepts` — predefined submodel templates / concept descriptions (e.g. ZVEI, Digital Nameplate) used for recognition and validation.

**Shared application logic**
- `AasxPackageLogic` — UI-agnostic application logic shared by the WPF and Blazor front ends: dialog helpers (`DispEditHelper*.cs`), menu/command handling, the scripting bridge (`AasxScript.cs`), main-window logic (`MainWindowLogic.cs`, `MainWindowTools.cs`, `MainWindowHeadless.cs`).
- `AnyUi` / `AasxIntegrationBase*` — a platform-agnostic UI abstraction ("AnyUI") and the plugin interface; see `.agents/architecture.md` for how this ties WPF and Blazor together.

**Front ends**
- `AasxPackageExplorer` — the WPF desktop application (entry point, `MainWindow.xaml`, menu bindings).
- `BlazorExplorer` / `BlazorUI` — the Blazor-based web UI.

**Plugins**
- `AasxPlugin*` (17 projects) — optional, dynamically loaded extensions, each usually implementing one IDTA submodel template's UI (technical data, digital nameplate, document shelf, BOM structure, contact information, plotting, MTP viewer, OPC UA client/server, etc.). See `.agents/plugins.md` for the full catalog and how to add a new one; `src/AasxPackageExplorer/plugins/README.md` for the runtime plugin-loading convention.

**CLI / server tools**
- `AasxToolkit` — command-line tool for AASX generation/extraction/validation.
- `AasxRestConsoleServer`, `AasxFileServerRestLibrary` — REST server hosting for `.aasx` files.
- `AasxSignature` — package signing utilities.

**Tests and docs**
- `*.Tests` / `*.GuiTests` projects — NUnit test suites next to the code they cover.
- `docdev/docfx_project` — DocFX developer documentation source (published via the `generate-doc` CI workflow).
...
For deeper detail than fits here, see:
- `.agents/architecture.md` — AnyUI abstraction, plugin interface, scripting engine.
- `.agents/plugins.md` — full plugin catalog.
- `.agents/ci-and-style.md` — exhaustive CI check details and exclude lists.
...
test` if Windows isn't available. Say so explicitly rather than claiming
a full build/run succeeded if you couldn't actually verify it.
...
./CheckBiteSized.ps1     # max line length 240 chars
...
fails. `InspectCode.ps1` additionally runs JetBrains ReSharper
(`dotnet jb inspectcode`) over the whole solution in CI.
...
projects are excluded from these checks — see `.agents/ci-and-style.md`
for the full exclude lists before assuming a file must comply.
...
This repo is set up so multiple AI coding tools can work here with
consistent context:

| Tool | Reads |
|---|---|
| Claude Code | `CLAUDE.md` (imports this file) + `.claude/settings.json` + `.claude/agents/*` |
| Codex CLI, Amp, Jules, RooCode, opencode, and other `AGENTS.md`-native tools | this file directly |
| GitHub Copilot | `.github/copilot-instructions.md` (mirrors this file) |
| Cursor | `.cursor/rules/*.mdc` (mirrors this file) |

If you're a tool not listed above and you can read this file, treat it as
your primary instructions. `.agents/` holds optional deep-dive docs linked
from the sections above — no directory-scanning convention is assumed, so
this file is written to stand on its own even if you never open that
folder.
